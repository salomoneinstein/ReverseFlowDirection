...
/***************************************************************************
 ReverseFlow
                                 A QGIS plugin
 This Plugin changes the direction of a polyline feature.
...
from .reverse_flow_dialog import ReverseFlowDialog
import os.path
...
class ReverseFlow:
    ...
    def tr(self, message):
        ...
        return QCoreApplication.translate('ReverseFlow', message)
    ...
    def initGui(self):
        ...
        icon_path = ':/plugins/reverse_flow/icon.png'
        self.add_action(
            icon_path,
            text=self.tr(u'Changes direction polyline feature'),
            callback=self.run,
            parent=self.iface.mainWindow())

        # will be set False in run()
        self.first_start = True
        self.clickTool.canvasClicked.connect(self.changeFlow)
    ...
    def unload(self):
        ...
        for action in self.actions:
            self.iface.removePluginMenu(
                self.tr(u'&Reverse Flow Direction'),
                action)
    ...
    def run(self):
        ...
        # show the dialog
        self.dlg.show()
        self.dlg.cbxLayers.setFilters(QgsMapLayerProxyModel.LineLayer)
    ...
    def changeFlow(self, point, button):	
        """Changes the direction of a polyline feature"""
        pntGeom = QgsGeometry.fromPointXY(point)
        pntBuff = pntGeom.buffer((self.map.mapUnitsPerPixel() * 2),0)
        rect = pntBuff.boundingBox()
		     		
        self.cLyr = self.dlg.cbxLayers.currentLayer()
        
        if not self.cLyr:
            QMessageBox.critical(None,"Missing layer","You must select a layer")
        else:
			
            cFld = self.dlg.cbxFields.currentField()
            
            if not cFld:
                QMessageBox.critical(None,"Missing field","You must select a field")
            else:                                    
                self.cLyr.removeSelection()
                self.cLyr.selectByRect(rect)

                sf = self.cLyr.selectedFeatures()
            
                if not sf:
                    QMessageBox.critical(None,"Missing feature","You must select a feature")
                else:
                    self.dlg.txtFeature.setText( \
                        str(sf[0].attribute(sf[0].fieldNameIndex(cFld))))
                    
                    geom = QgsGeometry(sf[0].geometry().constGet().reversed())
                    vrtx = len(sf[0].geometry().asMultiPolyline()[0])                   

                    self.dlg.txtVertex.setText(str(vrtx))                    
                    self.dlg.txtLength.setText(str(round(geom.length(),8)))
                                    
                    with edit(self.cLyr):
                        self.cLyr.changeGeometry(sf[0].id(),geom) 
    ...
    def clearDlg(self):
        ...
        if not self.cLyr:
            ...
            self.dlg.txtFeature.clear()
            self.dlg.txtVertex.clear()
            self.dlg.txtLength.clear()
            self.cLyr.removeSelection()
